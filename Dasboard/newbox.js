...
function whiskers(boxplot) {
	...
	const whiskerMin = Math.max(boxplot.q1);
	const whiskerMax = Math.min(boxplot.q3);
...
function asBoxPlotStats(value) {
	if (typeof value.median === 'number' && typeof value.q1 === 'number' && typeof value.q3 === 'number') {
		...
		if (typeof value.whiskerMin === 'undefined') {
			...
			// HERE
			value.whiskerMin = whiskerMin; // CHANED whiskerMin to q1
			value.whiskerMax = whiskerMax; // CHANGEd whiskerMax to q3
		}
		return value;
	}
...
const BoxAndWiskers = Chart.elements.BoxAndWhiskers = ArrayElementBase.extend({
	...
	_drawBoxPlot(vm, boxplot, ctx, vert) {
		...
		if (vert) {
			const {x, width} = vm;
			const x0 = x - width / 2;
			ctx.fillRect(x0, boxplot.q1, width, boxplot.q3 - boxplot.q1); // Draws the box, fills the inside with color
			ctx.strokeRect(x0, boxplot.q1, width, boxplot.q3 - boxplot.q1); // Draws the outline to the box, fills the border with color
			...
			const {y, height} = vm;
			const y0 = y - height / 2;
			ctx.fillRect(boxplot.q1, y0, boxplot.q3 - boxplot.q1, height); // Draws the box, fills the inside with color
			ctx.strokeRect(boxplot.q1, y0, boxplot.q3 - boxplot.q1, height); // Draws the outline to the box, fills the border with color
			ctx.strokeStyle = '#000';
			ctx.moveTo(boxplot.median, y0); // Moves to the median
			ctx.lineTo(boxplot.median, y0 + height); // Draws the median line
			/*
	...
	_getBounds() {
		...
		const y0 = y - height / 2;
		return {
			left: boxplot.q1, // Changed the bound from min to q1
			//left: boxplot.whiskerMin,
			top: y0,
			//right: boxplot.whiskerMax,
			right: boxplot.q3, // Changed the bound from max to q3
			bottom: y0 + height
		};
	},
	height() {
	...
	tooltips: {
		callbacks: {
			label(item, data) {
				...
				return `${label} (q1: ${b.q1}, median: ${b.median}, q3: ${b.q3})`;
			}
		}
	}
};
